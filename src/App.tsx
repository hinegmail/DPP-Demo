        ...
        <nav className="flex items-center gap-6 border-b border-gray-200 mb-8 overflow-x-auto no-scrollbar">
          {TABS.map((tab) => (
            <button
              ...
              className={`pb-4 text-sm font-medium transition-all relative whitespace-nowrap ${activeTab === tab.id ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'
                }`}
            >
              {tab.label}
              {activeTab === tab.id && (
        ...
        <AnimatePresence mode="sync">
          {activeTab === 'basic' && (
            <motion.div
              ...
              className="space-y-6"
            >
              {/* Top Section: Info + Product Image */}
              <div className="flex flex-col lg:flex-row gap-6">
                {/* Right Side: Product Image Display */}
                <div className="lg:w-1/3">
                  <div className="bg-white rounded-3xl border border-blue-100 shadow-sm overflow-hidden h-full flex flex-col group">
                ...
                {/* Left Side: Core Info Grid */}
                <div className="lg:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <InfoCard
                    label="产品名称 / Product Name"
                    value="槿木"
                    subValue="Muxu Premium Silk"
                  />
                  <InfoCard
                    label="品牌 / Brand"
                    value="在宥"
                    subValue="Authentic Brand"
                  />
                  <InfoCard
                    label="产品SKU / Product SKU"
              ...
              {/* Technical Stats & Additional Info Grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                <StatCard icon={<Calendar className="text-blue-500" />} label="制造日期 / Mfg Date" value={mockDPP.materials.garmentManufacturing.manufacturingDate} />
                <StatCard icon={<Layers className="text-indigo-500" />} label="设计研发 / R&D Site" value="在宥创新实验室" />
                <StatCard icon={<Weight className="text-blue-500" />} label="产品重量 / Weight" value="180g ± 5g" />
                <StatCard icon={<Zap className="text-yellow-500" />} label="绿电占比 / Energy" value={`${mockDPP.materials.garmentManufacturing.renewableEnergyRatio}%`} />
                <StatCard icon={<Leaf className="text-green-500" />} label="回收潜力 / Recycling" value="High (95%)" />

                <StatCard icon={<Database className="text-slate-500" />} label="数据来源 / Source" value="Enterprise Verified" />
                <StatCard icon={<FileText className="text-gray-500" />} label="安全类别 / Safety" value={`GB 18401-${mockDPP.product.gb18401Category}`} />
                <StatCard icon={<Thermometer className="text-red-500" />} label="洗涤温度 / Temp" value="≤ 40°C" />
                <StatCard icon={<Activity className="text-purple-500" />} label="预期寿命 / Life" value="> 50 Washes" />
...
function TimelineItem({ icon, label, value, active = false }: { icon: React.ReactNode; label: string; value: string; active?: boolean }) {
  return (
    <div className="flex items-start gap-6 group">
      <div className={`z-10 w-9 h-9 rounded-full flex items-center justify-center border-2 transition-colors ${active ? 'bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white border-gray-100 text-gray-300'
        }`}>
        {icon}
      </div>
      <div className="pt-1">
